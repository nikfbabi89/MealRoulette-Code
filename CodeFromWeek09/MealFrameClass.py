...
class MealGUI(tk.Frame): #The acting frame. Who knew inheritance was so useful.
    def __init__(self, Window=None, Whitelistbox=None, Blacklistbox=None): #Default None
        ...
        self.Instr = 'Left click an entry in the Meal List to select it. Right click while selected to filter that meal out, do this again to undo toggle. Left click it in the Selected Meals List to remove it from your listed meals. Use tabs to navigate to the filter screen.'
        ...
        self.MealList.bind('<ButtonRelease-1>', lambda event: Fotgui.GrabSelection(event, self.SelectedMeals, False)) #First Binding to grab selection.
        self.MealList.bind('<ButtonRelease-1>', lambda event: Fotgui.GrabSelection(event, self.Display, False), add='+') #2nd binding to grab selection twice.
        ...
        self.SelectedMeals.bind('<ButtonRelease-1>', lambda event: Fotgui.GrabSelection(event, self.SelectedMeals, True))
